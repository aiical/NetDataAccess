...
namespace NetDataAccess.Extended.Linkedin.KeyWordsSearchAtLinkedin
{
    ...
    public class GetLinkedinDetailPage : ExternalRunWebPage
    {
        ...
        public override void BeforeGrabOne(string pageUrl, Dictionary<string, string> listRow, bool existLocalFile)
        {
            ...
            if (this.CheckNeedLogin())
            {
                ...
                {
                    _UserToLastRunDates.Add(_CurrentUserIndex, DateTime.Now.Date.ToString("yyyyMMdd"));
                    this.Login();
                }
                else
                {
        ...
        private Dictionary<int, string> _UserToLastRunDates = new Dictionary<int, string>();

        private bool CheckIsNextDay(int index)
        {
            if (_UserToLastRunDates.ContainsKey(index))
            {
                string lastRunDate = _UserToLastRunDates[index]; 
                if (lastRunDate != DateTime.Now.Date.ToString("yyyyMMdd"))
                {
                    return true;
                }
                else
        ...
        private bool CheckCanLogin()
        {
            ...
            //else if(DateTime.Now.Date == 13)
            else if(this.CheckIsNextDay(0))
            {
