...
{
    public class TTGSListPageUrl : CustomProgramBase
    {
        ...
        private bool GetAllListPageUrl(IListSheet listSheet)
        {
            ...
            string resultFilePath = Path.Combine(exportDir, "沱沱工社获取所有列表页.xlsx");
            ...
            for (int i = 0; i < listSheet.RowCount; i++)
            {
                ...
                if (!giveUp)
                {
                    ...
                    try
                    {
                        ...
                        HtmlNode pageNode = htmlDoc.DocumentNode.SelectSingleNode("//div[@class=\"tlist_pxnum\"]/span");
                        if (pageNode == null)
                        {
                            this.RunPage.InvokeAppendLogText(category1Name + "-" + category3Name + "-" + category3Name + "，网页无法打开.", LogLevelType.Error, true);
                        }
                        else
                        {
                            ...
                            for (int j = 0; j < pageCount; j++)
                            {
                                ...
                                string detailPageUrl = "http://www.tootoo.cn/list-s1-" + category3Code + "-0-0-0-0-0-" + pageIndexStr + "-0-0-0-1,2,3,10201-zh_cn.html";
