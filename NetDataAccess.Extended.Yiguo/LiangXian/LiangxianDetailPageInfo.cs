...
{
    public class LiangxianDetailPageInfo : CustomProgramBase
    {
        ...
        private bool GeneratePageInfo(IListSheet listSheet)
        {
            ...
            Dictionary<string, int> resultColumnDic = CommonUtil.InitStringIndexDic(new string[]{
                ...
                "地区",
                "商品页URL"});
            string resultFilePath = Path.Combine(exportDir, "两鲜网商品详情" + DateTime.Now.ToString("yyyyMMdd") + ".xlsx");
            ...
            for (int i = 0; i < listSheet.RowCount; i++)
            {
                ...
                if (!giveUp)
                {
                    ...
                    try
                    {
                        ...
                        HtmlNode nameNode = htmlDoc.DocumentNode.SelectSingleNode("//div[@class=\"product-name\"]/h2");
                        name = CommonUtil.HtmlDecode(nameNode.InnerText.Trim());

                        HtmlNode priceNode = htmlDoc.DocumentNode.SelectSingleNode("//div[@class=\"price-box\"]/span/span[@class=\"price\"]");
                        if (priceNode != null)
                        {
                        ...
                        {
                            HtmlNode sPriceNode = htmlDoc.DocumentNode.SelectSingleNode("//div[@class=\"price-box\"]/p[@class=\"special-price\"]/span[@class=\"price\"]");
                            if (sPriceNode != null)
                            {
                                string priceStr = CommonUtil.HtmlDecode(sPriceNode.InnerText.Trim());
                                decimal testJiage = 0;
                                if (decimal.TryParse(priceStr.Substring(1), out testJiage))
                                {
                                    jiage = testJiage;
                        ...
                        HtmlNodeCollection allPropertyNodes = htmlDoc.DocumentNode.SelectNodes("//div[@class=\"product-box\"]/p");
                        if (allPropertyNodes != null)
                        {
                            foreach (HtmlNode propertyNode in allPropertyNodes)
                            {
                                HtmlNodeCollection allSpanNodes = propertyNode.SelectNodes("./span");
                                if (allSpanNodes != null)
                                {
                                    foreach (HtmlNode pNode in allSpanNodes)
                                    {
                                        string propertyText = CommonUtil.HtmlDecode(pNode.InnerText.Trim()).Trim();
                                        if (propertyText.StartsWith("规格："))
                                        {
                                            guige = propertyText.Substring(3).Trim();
                                        }
                                        else if (propertyText.StartsWith("产地："))
                                        {
                                            chandi = propertyText.Substring(3).Trim();
                                        }
                                    }
                                }
                                else
                                {
                                    string propertyText = CommonUtil.HtmlDecode(propertyNode.InnerText.Trim()).Trim();
                                    if (propertyText.StartsWith("规格："))
                                    {
                                        guige = propertyText.Substring(3).Trim();
                                    }
                        ...
                        HtmlNodeCollection allBundleNodes = htmlDoc.DocumentNode.SelectNodes("//ul[@class=\"options-list\"]/li/span/label/a");
                        if (allBundleNodes == null)
                        {
                        ...
                        {
                            foreach (HtmlNode bundleNode in allBundleNodes)
                            {
                                bundle = CommonUtil.HtmlDecode(bundleNode.FirstChild.InnerText.Trim()).Trim();
                                HtmlNode bundlePriceNode = bundleNode.SelectSingleNode("./span[@class=\"price-notice\"]/span[@class=\"price\"]");
                                if (bundlePriceNode == null)
                                {
                                    bundlePriceNode = bundleNode.SelectSingleNode("./span[@class=\"special-price\"]/span[@class=\"price\"]");
                                }
                                string priceStr = CommonUtil.HtmlDecode(bundlePriceNode.InnerText.Trim());
                                jiage = decimal.Parse(priceStr.Substring(1));
                                Dictionary<string, object> f2vs = new Dictionary<string, object>();
                                f2vs.Add("一级分类编码", category1Code);
                                f2vs.Add("二级分类编码", category2Code);
                                f2vs.Add("三级分类编码", category3Code);
