...
{
    public class PutianDetailPageUrl : CustomProgramBase
    {
        ...
        private bool GetAllDetailPageUrl(IListSheet listSheet)
        {
            ...
            string resultFilePath = Path.Combine(exportDir, "莆田网获取所有详情页.xlsx");
            ...
            for (int i = 0; i < listSheet.RowCount; i++)
            {
                ...
                { 
                    ...
                    try
                    {
                        ...
                        HtmlNodeCollection allDetailPageNodes = htmlDoc.DocumentNode.SelectNodes("//div[@class=\"product-list clearfix\"]/ul/li/h3/a");

                        if (allDetailPageNodes != null)
                        {
                            foreach (HtmlNode detailPageNode in allDetailPageNodes)
                            {
                                string goodDetailPageUrl = detailPageNode.Attributes["href"].Value;
                                int beginIndex = goodDetailPageUrl.IndexOf("-");
                                int endIndex = goodDetailPageUrl.IndexOf(".html");
                                string productSysNo = goodDetailPageUrl.Substring(beginIndex + 1, endIndex - beginIndex - 1);
                                string detailPageName = productSysNo + "_" + district;
                                string detailPageUrl = goodDetailPageUrl + "&c=" + district;
