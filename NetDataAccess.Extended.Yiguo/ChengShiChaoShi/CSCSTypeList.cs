...
{
    public class CSCSTypeList : CustomProgramBase
    {
        ...
        { 
            for (int i = 0; i < listSheet.RowCount; i++)
            {
                ...
                try
                {
                    ...
                    HtmlNodeCollection allCategory1Nodes = htmlDoc.DocumentNode.SelectNodes("//div[@class=\"box all-category\"]/div[2]/ul[1]/li");
                    if (allCategory1Nodes != null)
                    {
                        for (int j = 0; j < allCategory1Nodes.Count; j++)
                        {
                            ...
                            string category1Code = category1Url.Substring(category1Url.LastIndexOf("/") + 1);
                            HtmlNodeCollection allCategory2Nodes = category1Node.SelectNodes("./div[1]/dl");
                            if (allCategory2Nodes != null)
                            {
                                for (int k = 0; k < allCategory2Nodes.Count; k++)
                                {
                                    HtmlNode category2Node = allCategory2Nodes[k];
                                    HtmlNode category2NameNode = category2Node.SelectSingleNode("./dt/a");
