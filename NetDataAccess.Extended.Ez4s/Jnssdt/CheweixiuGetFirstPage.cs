...
namespace NetDataAccess.Extended.Ez4s
{
    ...
    /// 车维修获取首页
    /// </summary>
    public class CheweixiuGetFirstPage : CustomProgramBase
    {
        #region 入口函数
        /// <summary>
        /// 入口函数
        ...
        { 
            ...
            string resultFilePath = Path.Combine(exportDir, "车维修获取列表页.xlsx");
        ...
        private void GetAllListPageUrl(IListSheet listSheet, string pageSourceDir, ExcelWriter resultEW)
        {
            for (int i = 0; i < listSheet.RowCount; i++)
            {
                ...
                try
                {
                    ...
                    //获取导航栏元素
                    HtmlNodeCollection allListPageLinkNodes = htmlDoc.DocumentNode.SelectNodes("//p[@class=\"m-2\"]/a");
                    if (allListPageLinkNodes != null)
                    {
                        //获取显示最大页码数的那个元素
                        HtmlNode lastPageLinkNode = allListPageLinkNodes[allListPageLinkNodes.Count - 2];
                        ...
                        {
                            int pageIndex = j + 1;
                            string url = pageUrl + "p" + pageIndex.ToString() + "/";
                            Dictionary<string, string> f2vs = new Dictionary<string, string>();
                            f2vs.Add("detailPageUrl", url);
                            f2vs.Add("detailPageName", district + pageIndex.ToString());
