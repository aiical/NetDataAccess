...
namespace NetDataAccess.Extended.Tanyin
{
    /// <summary>
    /// 节能搜索低碳获取详情页
    /// 运行此程序前，系统已经提前爬取了listSheet中指定的详情页html，
    /// 然后此扩展程序解析这些详情页，形成结构化信息存储在excel中，
    /// 并从详情信息html获取展会信息正文中的img图片url
    /// </summary>
    public class JnssdtGetDetail : CustomProgramBase
    {
        #region 入口函数
        /// <summary>
        /// 入口函数
        ...
        {
            return GetAllPageDetailInfo(listSheet) && GetAllPageImgUrl(listSheet);
        }
        #endregion
        ...
        private bool GetAllPageDetailInfo(IListSheet listSheet)
        {
            ...
            //输出文件地址
            string resultFilePath = Path.Combine(exportDir, "新产品新技术-节能搜索低碳.xlsx");
            ...
            for (int i = 0; i < listSheet.RowCount; i++)
            {
                ...
                if (!giveUp)
                {
                    ...
                    try
                    {
                        ...
                        HtmlNode propertyNode = htmlDoc.DocumentNode.SelectSingleNode("//div[@class=\"fx_lyu\"]/span");
                        string propertyValue = CommonUtil.HtmlDecode(propertyNode.InnerText).Trim();
                        int authorBeginIndex = propertyValue.IndexOf("来源：") + 3;
                        int authorEndIndex = propertyValue.IndexOf("评论：");
                        string author = authorBeginIndex >= 0 && authorEndIndex >= 0 ? propertyValue.Substring(authorBeginIndex, authorEndIndex - authorBeginIndex).Trim() : "";

                        //HtmlDocument对象中获取展会信息正文HTML，注意这里是取的InnerHtml
                        HtmlNode contentNode = htmlDoc.DocumentNode.SelectSingleNode("//div[@class=\"content_nr\"]");
        ...
        #region 逐个详情页获取图片url
        private bool GetAllPageImgUrl(IListSheet listSheet)
        {
            string exportDir = this.RunPage.GetExportDir();
            string pageSourceDir = this.RunPage.GetDetailSourceFileDir();
            ...
            //此输出文件是项目“节能搜索低碳获取图片”的输入文件
            string resultFilePath = Path.Combine(exportDir, "节能搜索低碳获取图片.xlsx");
            ExcelWriter resultEW = new ExcelWriter(resultFilePath, "List", resultColumnDic, null);
