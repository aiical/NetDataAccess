...
{
    /// <summary>
    /// TongliaoQiPingDangan
    /// </summary>
    public class TongliaoQiPingDangan : ExternalRunWebPage
    {
        ...
        private bool GenerateDAGLJL(IListSheet listSheet)
        {
            ...
            for (int i = 0; i < listSheet.RowCount; i++)
            {
                ...
                try
                {
                    ...
                    HtmlNodeCollection listTrNodeList = htmlDoc.DocumentNode.SelectNodes("//table[@class=\"table checkbox\"]/tr");
                    if (listTrNodeList.Count > 1)
                    {
                        for (int j = 1; j < listTrNodeList.Count; j++)
                        {
                            ...
                            reportInfo.Add("气瓶登记证编号", vNodeList[3].InnerText.Trim());
                            reportInfo.Add("车牌号", vNodeList[4].InnerText.Trim());
                            reportInfo.Add("安装数量", vNodeList[5].InnerText.Trim());
                            reportInfo.Add("用户名", vNodeList[6].InnerText.Trim());
                ...
                try
                {
                    ...
                    if (listTrNodeList.Count > 1)
                    {
                        for (int j = 1; j < listTrNodeList.Count; j++)
                        {
                            ...
                            {
                                string pageUrl = "http://218.56.62.250/jingmen/VehicleGas/VehicleGasView.aspx?RegId=" + rId;
                                rIdToNull.Add(rId, ""); 
