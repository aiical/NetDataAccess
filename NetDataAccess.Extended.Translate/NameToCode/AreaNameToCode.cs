...
namespace NetDataAccess.Extended.Translate.NameToCode
{
    /// <summary>
    /// 获取名字到编码
    /// </summary>
    public class AreaNameToCode : ExternalRunWebPage
    { 
        ...
        private void LoadDictionary()
        {
            ...
            for (int i = 0; i < rowCount; i++)
            {
                ...
                string toCode = row["国际域名缩写"];
                string cnName = row["中文名"];
                string enName = row["英文名"];
                this.InsertToDic(enName, toCode, dicValues); 
                this.InsertToDic(cnName, toCode, dicValues);
                ...
                if (qtywmName.Length != 0)
                {
                    ...
                    {
                        this.InsertToDic(qtywmNames[j], toCode, dicValues);
                    }
                }
            }
            this._Dic = dicValues;
        }
        private void InsertToDic(string fromName, string toCode, Dictionary<string, string> dic)
        {
            fromName = fromName.ToLower().Trim();
            if (fromName.Length != 0 && !dic.ContainsKey(fromName))
            {
                dic.Add(fromName.Trim(), toCode.Trim());
            }
        }
    }
}